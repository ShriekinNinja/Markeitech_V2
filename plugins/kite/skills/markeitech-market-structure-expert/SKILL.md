...
description: Review a specific swing, pivot, leg, level, FVG, zone, or profile definition and its point-in-time behavior. Price geometry does not establish order flow or trading edge.
---
...
- Use the accepted entity definition, exact input contract, session, timeframe, and confirmation time.
- Check future leakage, warmup, ties, overlap, revisions, invalidation, and expiry only where relevant to the requested entity.
- Separate formation, later interaction, and interpretation; a right-span pivot is not known at its pivot timestamp.
- Do not infer participant intent, future fills, or tradeability from geometry or a chart match.

## Optional References

- [domain-contract.md](references/domain-contract.md) — use for the matching entity or causal-boundary question.
- [source-census.md](references/source-census.md) — use for an external market-structure definition is material.
