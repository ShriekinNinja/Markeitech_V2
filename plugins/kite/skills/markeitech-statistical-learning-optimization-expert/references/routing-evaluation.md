# Statistical Learning Advisor Routing Evaluation
...
|---|---|---|---|---|
| Verify a metric formula, session boundary, revision rule, lineage, or fidelity | Data quality for source/time/lineage; quantitative validation for formula/aggregation; evidence fitness only for a named downstream use | Statistical advisor does not redefine analytical truth | ALIGNED | PENDING |
...
| Change feature or label storage, retention, or licensing | Architecture/persistence/provider or licensing owner before statistical work | No implicit raw retention, schema, or license approval | ALIGNED | PENDING |
| Permit automatic retraining or live parameter mutation | Governance/policy owner; statistical learning may review evidence | An alarm or experiment never grants adaptation authority | ALIGNED | PENDING |
| Feed a score to Sir Loke | Statistical learning for score validity; live-agent governance for permissible use | Model evidence and agent authority remain separate | ALIGNED | PENDING |
| Review a deterministic metric with no model or optimization use | Data quality and quantitative validation; evidence fitness only when a named downstream use is being admitted | Avoid unnecessary statistical routing | ALIGNED | PENDING |

Fresh-thread acceptance must record observed advisor selection, order, skill loading, evidence
labels, proportional output, and whether missing specialist coverage stops only the affected
conclusion. Static wording or successful installation alone is not measured routing behavior.
