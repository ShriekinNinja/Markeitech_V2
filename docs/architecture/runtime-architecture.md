...
Native tick writes do not synchronously consolidate overlapping Parquet file ranges. The SQLite identity ledger removes already committed events before a normal write, exact crash retries verify the complete durable identity set, and Markeitech native-tick readers defensively deduplicate logical identities. This keeps potentially expensive Nautilus file consolidation outside the live writer and shutdown deadline. Canonical bars remain synchronously consolidated because their volume is bounded and they directly support startup recovery. Native tick file consolidation is offline maintenance debt and must be completed before a future replay path reads the Nautilus catalog directly without the Markeitech adapter.
