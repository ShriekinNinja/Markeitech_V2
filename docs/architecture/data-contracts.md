...
Matches are first partitioned into coherent price clusters by their zone bounds plus applied tolerance. Qualification requires the configured number of distinct source kinds inside one cluster, not across unrelated prices traversed by the same candle and not merely from several levels of one kind. The selected cluster is deterministic and retains decomposed quality evidence: source and timeframe diversity, exact touches, reported versus inferred/partial contributors, and mean normalized distance. Alternate clusters remain inspectable; they cannot silently contribute to the selected setup.

Existing but unmatched sources return `not_at_location`; clusters below confluence return `insufficient_confluence`; unavailable policy/current-clock evidence returns `missing_evidence`. Missing configured timeframes or source payloads mark the result degraded. Nested level/FVG timestamps newer than their committed feature fail closed as look-ahead evidence.
...
Meaningful interaction-state changes are represented by immutable `LocationInteractionEvent` records. Each event binds the episode and definition identity to its completed observation, entry evidence, selected quality cluster, interaction state, and favorable/adverse confirmation progress. Events attached to lifecycle entry, replacement, or exit commit atomically with those lifecycle mutations; other interaction changes use an idempotent standalone write. Restart restores the latest durable event for each open episode so pending rejection or acceptance confirmation cannot be forgotten or manufactured by a process restart. Repeated observations that leave the interaction state unchanged do not create durable event noise.
