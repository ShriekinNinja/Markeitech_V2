...
This separation preserves ownership and revision semantics. Nautilus continues to own runtime connectivity and its native tick/bar catalog behavior; Markeitech owns feature identity and storage. Parquet is durable feature payload truth. A later SQLite manifest records committed feature identities and monotonic processing checkpoints only after catalog durability, and a bounded asynchronous sink keeps those writes off the actor callback thread. JSONL logs do not substitute for either layer.
