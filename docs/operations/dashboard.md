...
| `acquisition_retry_interval_ms` | `1000` | 100–10000; membership, attachment and busy history-admission retry cadence |
...
win at the browser display boundary. Terminal dashboard request metadata is retired after
publication; recent terminal IDs have a bounded duplicate-protection window. Raw inputs are
discarded after projection, with no new persistence or dependencies.
