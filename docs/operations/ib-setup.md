...
The PyCharm `Market Data - Continuous Live Context` run configuration invokes this guarded command with `config/market-data.local.toml`. Unlike the acceptance command, it has no duration limit and runs until stopped. Its emulated terminal delivers `Ctrl+C`, and the entry point translates `SIGINT`, `SIGTERM`, and terminal-close `SIGHUP` into `TradingNode.stop_async()`. One stop request should print `SHUTDOWN_REQUESTED`, enter the Nautilus stopping lifecycle, drain Markeitech's managed workers, and return control. After the runner returns, its final JSON reports `status: stopped` and the handled signal in `result`; `status: started` is reserved for the synchronous test boundary that returns immediately after starting its supplied node. The `[live_node]` post-stop, disconnection, and final-shutdown ceilings default to a seven-second aggregate budget so PyCharm does not force-terminate the process during Nautilus's settle period. A second forced stop is reserved for a shutdown that remains stuck beyond that bounded drain. The console emits bounded `OPERATOR_CONTEXT`, `OPERATOR_LEVELS`, and `OPERATOR_AUCTION` reports while detailed `MARKET_CONTEXT_EVENT` records remain available in file logs. See the [operator context log guide](operator-context-logs.md).
