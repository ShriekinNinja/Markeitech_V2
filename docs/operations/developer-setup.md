...
The current loader accepts only system schema **27**. To migrate a schema-23/24 local profile:
...
5. Set `schema_version = 27` and compare the result with the corresponding tracked profile.

For a schema-25/26 profile, set `schema_version = 27` and optionally copy the documented
`[dashboard]` section from `system.example.toml`. Omission keeps it disabled. All dashboard
settings are startup-only. An existing dashboard section must set `policy_version = 2`;
