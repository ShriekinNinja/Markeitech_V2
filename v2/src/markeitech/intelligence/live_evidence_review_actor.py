...
class LiveEvidenceReviewActor(DataActor):
    ...
    def on_data(self, data) -> None:  # noqa: ANN001
        ...
        self.clock.set_time_alert_ns(
            _FREEZE_ALERT,
            self.clock.timestamp_ns() + self._coalescing_ns,
            callback=self._freeze,
        )
        self.log.info(
            "LIVE_EVIDENCE_REVIEW_CAPTURE_TRIGGERED"
            f" | interval_end_ns={payload.interval_end_ns} | revision={payload.revision}",
        )
    ...
    def on_signal(self, signal: Signal) -> None:
        ...
        if signal.name == SYSTEM_HEALTH_SIGNAL:
            ...
            if health.state in {"FAILED", "STOPPING"} and self._state in {
                "WAITING_FOR_OPERATIONAL_COMPLETION", "WAITING_FOR_LIVE_ES_5M", "COALESCING",
            }:
    ...
    def on_stop(self) -> None:
        ...
        for name in (_READINESS_DEADLINE, _LIVE_BAR_DEADLINE, _FREEZE_ALERT, _RESULT_TIMER):
