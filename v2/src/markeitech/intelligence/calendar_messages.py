...
CALENDAR_PROJECTION_RESPONSE_TYPE_NAME = "markeitech.calendar.projection.response.v1"
...
class CalendarProjectionResponse:
    ...
    def __post_init__(self) -> None:
        ...
        _schema(self.schema_version)
        if self.status not in {"READY", "NOT_READY", "REJECTED", "INCOMPLETE"}:
            raise ValueError("unsupported calendar projection response status")
        ids = tuple(item.calendar_id for item in self.projections)
        if len(ids) != len(set(ids)):
            raise ValueError("projection response contains duplicate calendars")
        if set(ids) & set(self.unavailable_calendar_ids):
            raise ValueError("calendar cannot be both projected and unavailable")
        if self.status == "READY" and self.unavailable_calendar_ids:
            raise ValueError("READY projection response cannot report unavailable calendars")
        if self.status in {"NOT_READY", "REJECTED"} and self.projections:
            raise ValueError(f"{self.status} projection response cannot contain projections")
...
def _schema(value: object) -> None:
    if value != CALENDAR_SCHEMA_VERSION:
        raise ValueError(f"unsupported calendar schema version: {value!r}")
